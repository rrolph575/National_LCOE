...
poi = gpd.read_file("/shared-projects/rev/projects/goMexico/data/vector/prioritzed_poi_update2.gpkg")
...
costSurfaceArray = raster2array("/shared-projects/rev/projects/goMexico/data/raster/costs_1500MW_clipped_tight.tif")
CostSurfacefn = "/shared-projects/rev/projects/goMexico/data/raster/costs_1500MW_clipped_tight.tif"

#starting at 901, because eagle randomly dropped the job
overall_cost_table = gpd.read_file("/shared-projects/rev/projects/goMexico/data/vector/lb_tx_results/call_area_lcp_901_done.gpkg")


#iterates through all starting points and finds LCP for each
for i in points['id'][901:]:
    ...
    print(f"done with coast point {i} in {round(end-start, 3)} seconds; {100 * round(i/max(points['id']), 3)}% done")
    if i%100 == 0:
        print("saving every 100 points")
        gpd.GeoDataFrame(overall_cost_table.reset_index(drop=True)).to_file(f"/shared-projects/rev/projects/goMexico/data/vector/lb_tx_results/call_area_lcp_{i+1}_done.gpkg")
        print("done saving")


#this is lines file, called paths.gpkg in deliverable, merged with the coast points in coast_point_costs.gpkg
gpd.GeoDataFrame(overall_cost_table.reset_index(drop=True)).to_file("/shared-projects/rev/projects/goMexico/data/vector/lb_tx_results/call_area_lcp_final.gpkg")